            ...
            if (NodeProto && !NodeProto._react_dom_shield_active) {
                ...
                NodeProto.removeChild = function(child) {
                    if (child.parentNode !== this) {
                        if (targetWin.console && targetWin.console.warn) {
                            targetWin.console.warn('[React DOM Shield] removeChild interceptado: el nodo no es hijo directo de este contenedor.', child, this);
                        }
                        return child;
                    }
                ...
                NodeProto.insertBefore = function(newNode, refNode) {
                    if (refNode && refNode.parentNode !== this) {
                        if (targetWin.console && targetWin.console.warn) {
                            targetWin.console.warn('[React DOM Shield] insertBefore interceptado: el nodo de referencia no es hijo directo de este contenedor.', refNode, this);
                        }
                        return newNode;
                    }
                ...
                NodeProto._react_dom_shield_active = true;
            }
        } catch (e) {
            console.warn('[React DOM Shield] Error al inicializar:', e);
        }
    })();
    </script>
    """,
...
<style>
    ...
    /* ========================================================================= */
    /* RUEDA ONDULADA DINÁMICA DE CARGA (CUSTOM WAVY LOADER SPINNER)             */
    /* ========================================================================= */
    div[data-testid="stSpinner"],
    div.stSpinner {
        display: flex !important;
    ...
    div.stSpinner > div {
        ...
        box-shadow: 0 12px 30px -6px rgba(11, 87, 208, 0.15), 0 6px 12px -4px rgba(11, 87, 208, 0.08) !important;
    ...
    /* Ocultar el icono y svg nativo de Streamlit */
    ...
    /* Inyectar la rueda ondulada giratoria de 12 ondas con degradé azul */
...
def renderizar_tablero_conciliacion_desafios(user_rol, user_sector, current_user):
    ...
    st.dataframe(styler_c, use_container_width=True, height=420, hide_index=True)
...
if 'lideres_creadas_log' in st.session_state and st.session_state['lideres_creadas_log']:
    with st.expander("✨ Cuentas de Nuevas Líderes Creadas Automáticamente", expanded=True):
        ...
        st.dataframe(df_l_log, use_container_width=True, hide_index=True)
...
def renderizar_modo_app(df_filtrado, user_rol, user_nombre, user_grupo, user_sector, is_dark_theme):
    ...
    with tab_app3:
        ...
        if df_filtrado.empty:
            ...
            st.dataframe(df_fact_view, use_container_width=True, height=450, hide_index=True)
...
if hasattr(st, 'dialog'):
    ...
    def dialog_origen_activas(df_fuente, sector_nombre, real_act, obj_act, cump_act):
        ...
        st.caption(
            "💡 **Diferencia técnica con Tableau:** En la pestaña *'📊 Informe Tableau Cam'*, se auditan las consultoras individuales con pedido en el corte específico de base. "
    ...
    @st.dialog("👥 Detalle de Consultoras y Líderes", width="large")
    def dialog_origen_lideres(df_fuente, sector_nombre, total_c):
        st.markdown(f"### 👥 Equipo de Líderes — {sector_nombre}")
        st.markdown(f"**Total Líderes Activas en el Sector:** `{total_c}`")
        st.info("📁 Este conteo corresponde a los grupos con Líder de Negocio formalmente asignada dentro del sector activo.")

        col_grp = next((c for c in df_fuente.columns if 'grupo' in str(c).lower()), 'Código de grupo')
        col_nom = 'Nombre de consultora' if 'Nombre de consultora' in df_fuente.columns else df_fuente.columns[0]

        df_l = df_fuente.copy()
        if col_nom in df_l.columns:
            df_l = df_l[df_l[col_nom].notna() & (~df_l[col_nom].astype(str).str.strip().str.lower().isin(['nan', 'none', '', '0']))]

        cols_l = [c for c in [col_grp, col_nom, 'Color', 'Disponibles', 'Real Activas'] if c in df_l.columns]
        df_l_view = df_l[cols_l].rename(columns={col_grp: 'Grupo', col_nom: 'Nombre Líder', 'Color': 'Nivel / Color'})
...
if es_tab_tableau_activo and user_rol == 'lider':
    ...
    with lkpi2:
        ...
        if st.button("drill_lider_act", key="btn_drill_lider_act", help="Pica aquí para auditar el origen oficial de tus activas"):
    ...
    with kpi1:
        ...
        if st.button("drill_lideres", key="btn_drill_lideres", help="Pica aquí para ver quiénes son las líderes"):
    ...
    with kpi2:
        ...
        if st.button("drill_activas", key="btn_drill_activas", help="Pica aquí para auditar el origen de las activas"):
    ...
    with kpi3:
        ...
        if st.button("drill_fact", key="btn_drill_fact", help="Pica aquí para auditar el desglose de facturación por líder"):
...
if not tabs_permitidas:
    ...
    tab_usuarios = tab_objs.get("tab_usuarios") if user_rol == 'superadmin' else None
    tab_lideres_gerente = tab_objs.get("tab_lideres_gerente") if user_rol == 'gerente' else None
            ...
            with tab_tab_niveles:
                ...
                with col_n1:
                    ...
                    if not df_calc_tab.empty and 'Color' in df_calc_tab.columns:
                        ...
                        cols_c_render = ['Nivel / Color', 'Total Consultoras', 'N° Activas', '% Actividad', 'Facturación Total', 'Productividad']
                        df_color_render = df_color_render[[c for c in cols_c_render if c in df_color_render.columns]].reset_index(drop=True)
...
if tab_geral is not None:
    with tab_geral:
        ...
        with st.spinner("Cargando cartera y registros de Crédito & Cobranza..."):
    ...
    with tab_resumen:
        ...
        with col_left:
            ...
            if 'Nombre Setor' in df_filtrado.columns:
                ...
                if agg_dict:
                    ...
                    st.dataframe(resumen_sector, use_container_width=True, hide_index=True)
                else:
        ...
        with col_right:
            ...
            if 'Color' in df_filtrado.columns:
                ...
                st.dataframe(resumen_color, use_container_width=True, hide_index=True)
...
if tab_ganancia is not None:
    with tab_ganancia:
        ...
        with col_mat:
            ...
            ).reset_index().rename(columns={'index': 'Activas / Facturación'})
        ...
        with col_pot:
            ...
            st.dataframe(df_pot_view, use_container_width=True, hide_index=True)
...
if tab_diagnostico is not None:
    with tab_diagnostico:
        ...
        if df_diag.empty:
            ...
            df_diag['Tipo_Red'] = df_diag.apply(_resolver_tipo_red_app, axis=1)
...
if tab_usuarios is not None and user_rol == 'superadmin':
    with tab_usuarios:
        ...
        with sub_tab_sub:
            ...
            if not df_res_sub.empty:
                ...
                st.dataframe(df_res_sub[cols_show_sub], use_container_width=True, hide_index=True)
        ...
        with sub_tab_users:
            ...
            with col_u1:
                ...
                st.dataframe(df_u_all, use_container_width=True, hide_index=True)
...
if tab_lideres_gerente is not None and user_rol == 'gerente':
